...
public final class PriceFeed {
    ...
    public record Px(BigDecimal price, BigDecimal usd, String source) {}
    ...
    private static Px fetch(String symbol) throws Exception {
        if ("btc".equals(symbol)) {
            String body = getBody("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=eur,usd");
            return new Px(extract(body, "\"eur\"\\s*:\\s*([0-9.]+)"),
                          extract(body, "\"usd\"\\s*:\\s*([0-9.]+)"), "live");
        }
        // AAPL: Yahoo quotes USD; the euros come through the FX service
