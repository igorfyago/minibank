...
/**
 * An in-memory directory for demos and smoke tests — the same tables the
 * Postgres schema defines, held in maps. Boot with SSO_DB_URL=memory.
 *
 * This is not a toy: it enforces the same unique constraint, the same
 * expiry check, the same revocation rule. What it does not do is survive
 * a restart — which is exactly what you want from a demo directory.
 */
public final class MemoryDb implements ConnectionSource {
    ...
    private PreparedStatement prepare(String sql) {
        return (PreparedStatement) java.lang.reflect.Proxy.newProxyInstance(
            ...
            new MemoryStatement(sql));
    }

    private final class MemoryStatement implements java.lang.reflect.InvocationHandler {
        private final String sql;
        private final Map<Integer, Object> params = new HashMap<>();

        MemoryStatement(String sql) { this.sql = sql; }
        ...
        private int executeUpdate() throws SQLException {
            if (sql.startsWith("INSERT INTO users")) {
                ...
                if (users.containsKey(email)) throw new SQLException("unique constraint violation");
